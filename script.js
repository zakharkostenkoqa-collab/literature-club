...
async function renderFigures() {
  const [figs, lib] = await Promise.all([
    loadJSON('data/figures.json'),
    loadJSON('data/library.json')
  ]);
  figuresData = figs;
  ...
  grid.innerHTML = figs.figures.map(f => {
    const books = byAuthor[f.id];
    if (!books) {
      return `
        <div class="figure-card figure-locked" title="Відкриється, коли клуб прочитає твір цього автора">
          <div class="locked-mark">?</div>
          <div class="locked-era">${f.era}</div>
          <div class="locked-hint">ще не відкрито</div>
        </div>`;
    }
