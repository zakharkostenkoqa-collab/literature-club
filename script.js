...
/* ---------- Мапа письменників ---------- */
async function renderWritersMap() {
...
async function renderParticipants() {
  ...
  const box = document.getElementById('participants-content');
  ...
  box.innerHTML = data.members.map(m => `
    <div class="participant-card">
      ${m.photo
        ? `<img class="participant-photo" src="${m.photo}" alt="${m.name}">`
        : `<div class="participant-photo participant-initials">${initials(m.name)}</div>`}
      <h3>${m.name}</h3>
      ${m.favorite ? `<p class="participant-favorite">${m.favorite}</p>` : ''}
    </div>`).join('');
}
