...
async function renderHeaderStats() {
  ...
  renderCountdown(manualStats.countdown);
}
...
function renderEraPanel(i) {
  ...
  const booksHTML = era.books.map(b => {
...
async function renderTimeline() {
  ...
  const doneCount = eras.filter(e => e.books && e.books.length).length;
  const bookCount = eras.reduce((s, e) => s + (e.books ? e.books.length : 0), 0);
  const pct = Math.round(doneCount / eras.length * 100);

  // Прогрес над стрічкою
  ...
  box.innerHTML = eras.map((e, i) => {
    const has = e.books && e.books.length > 0;
    const count = has ? e.books.length : 0;
    return `
