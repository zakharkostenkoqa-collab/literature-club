/* ============================================================
   ПЕРЕМИКАЧ РЕЖИМУ ДОПОВІДАЧА
   false — кнопка заблокована, показує «в розробці»
   true  — режим доступний усім
   Перед зустріччю поставте true і збережіть файл.
   ============================================================ */
const PRESENTER_ENABLED = false;
...
async function renderNextTopic() {
  ...
  box.innerHTML = `
    <div class="card next-topic-card">
      ...
      ${blocksHTML ? (PRESENTER_ENABLED
        ? `<button class="presenter-launch" onclick="openPresenter()">▶ Режим доповідача</button>`
        : `<button class="presenter-launch is-locked" disabled title="Відкриється перед зустріччю">▶ Режим доповідача<span class="launch-soon">в розробці</span></button>`) : ''}
    </div>`;
